...
class ZenodoClient:
    ...
    def get_latest_hflav(
        ...
        def record_date(rec: Dict[str, Any]) -> datetime:
            md = rec.get("metadata", {})
            date_str = (
                md.get("publication_date") or rec.get("created") or md.get("date")
            )
            if not date_str:
                return datetime.min
            # Accept several likely formats
        ...
        latest = max(hits, key=record_date)
        return latest
    ...
    def download_file(
        ...
        if isinstance(record_or_id, int):
            record = self.get_record(record_or_id)
        elif isinstance(record_or_id, dict):
            record = record_or_id
        else:
